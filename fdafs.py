...
def Get_surplus3(pool,olist,Traders,time,V):
    s = 0
    for neworder in olist:
        pool[neworder.ID] = neworder
    ...
    if lb*la != 0:
        ...
        for i in range(min(lb,la)):
            ...
            if opB[i].Price >= opA[i].Price:
                ...
                v1 = V[opB[i].Index][time]
                v2 = V[opA[i].Index][time]
                B_deadID.append(opB[i].ID)
                A_deadID.append(opA[i].ID)
                pairs.append(Order_pair(opB[i],opA[i],(opB[i].Price+opA[i].Price)/2,1,time,V))
    ...
    newpool = {}
    for newb in opB:
        if newb.ID not in B_deadID:
            newpool[newb.ID] = newb
            
    for newa in opA:
        if newa.ID not in A_deadID:
            newpool[newa.ID] = newa
            
    return s, newpool,pairs



def Fixed_list(nlists,n):
    gap = int(len(nlists)/n)
    rlists = []
    for ns in range(1,n):
        rlists.append(nlists[ns*gap])
        
    rlists.append(nlists[-1])
    return rlists



strategies7 = [[0,0,1],[0,20,1],[0,50,1],[0,100,1],[20,50,1],[20,100,1],[50,100,1]]
strategies4 = [[0,0,1],[0,20,1],[0,50,1],[0,100,1]]
strategies1 = [[0,0,1]]

def Simu2(maxprice,midprice,num,P, Value,strategies,stop_num):
    itera = 200
    Traders = {}
        ...
        for ii in agents:
            ...
            if arrive == 1:
                ...
                if Traders[ii].Type == 'Seller':
                    a = 'Ask'
                    p = min(valuation + demand,1000)
                    s = 1

                else:
                    a = 'Bid'
                    p = max(valuation - demand,0)
                    s = 1

                order = Order('Add','Limit',a,s,p,ii,step)
                #M.Write_into_Pool(order)
                #order.Valuation = Traders[ii].Valuation
                order_c = copy.deepcopy(order)
                slists.append(order_c)
                lists.append(order_c)
                Traders[ii].Place_Order(order)
                
            if inid == 1:
                
                smpool = copy.deepcopy(M.Order_Pool)
                oops = copy.deepcopy(slists)
                deltareal, newpools,result = Get_surplus3(smpool,oops,Traders,step,Value)
                realtot += deltareal
    ...
    print(asur,realtot,tot)
    print(realtot,tot,asur/tot)
...
f4 = open('fdafs.pkl', 'wb')  
pickle.dump(final, f4, -1) 
